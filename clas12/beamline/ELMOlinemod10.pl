...
sub ELMOlinemod10()
{
	...
	print_det(\%configuration, \%detector);
        ...
        my $nplanes_tredu = 2;
	$firstVacuumOR = 28.52;
	my @zplane_tredu  =  ($zplane_pb1[1], $zf_stube);
        my @iradius_tredu  = ($firstVacuumOR, $firstVacuumOR);
	my @oradius_tredu  = ($iradius_tcone[0], $iradius_tcone[0]);
	%detector = init_det();
	$detector{"name"}        = "ElmoTungsteneReduction";
	$detector{"mother"}      = "root";
	$detector{"description"} = "Tungsten reduction - ELMO configuration";
	$detector{"color"}       = "dd8648";
	$detector{"type"}        = "Polycone";
	$dimen = "0.0*deg 360*deg $nplanes_tredu*counts";
        for(my $i = 0; $i <$nplanes_tredu; $i++) {$dimen = $dimen ." $iradius_tredu[$i]*mm";}
        for(my $i = 0; $i <$nplanes_tredu; $i++) {$dimen = $dimen ." $oradius_tredu[$i]*mm";}
        for(my $i = 0; $i <$nplanes_tredu; $i++) {$dimen = $dimen ." $zplane_tredu[$i]*mm";}
        $detector{"dimensions"}  = $dimen;
        $detector{"material"}    = "beamline_W";
	$detector{"style"}       = 1;
	print_det(\%configuration, \%detector);
