...
sub ELMOlinemod8()
{
	...
	my $oradius_steel = 109.54;
        ...
        $detector{"style"}       = 1;
        print_det(\%configuration, \%detector);
	my @zplane_pb3  =  ($zplane_pb2[1]+30, $torusStart-20);
	my $iradius_pb3 = 117.5;
	my $oradius_pb3 = $iradius_pb3+14;
        %detector = init_det();
        $detector{"name"}        = "ElmoPbCylinder3";
        $detector{"mother"}      = "root";
        $detector{"description"} = "Lead Cylinder 3 - ELMO configuration";
        $detector{"color"}       = "999966";
        $detector{"type"}        = "Polycone";
        $dimen = "0.0*deg 360*deg $nplanes_pb*counts";
        for(my $i = 0; $i <$nplanes_ttip; $i++) {$dimen = $dimen ." $iradius_pb3*mm";}
        for(my $i = 0; $i <$nplanes_ttip; $i++) {$dimen = $dimen ." $oradius_pb3*mm";}
        for(my $i = 0; $i <$nplanes_ttip; $i++) {$dimen = $dimen ." $zplane_pb3[$i]*mm";}
        $detector{"dimensions"}  = $dimen;
        $detector{"material"}    = "G4_Pb";
        $detector{"style"}       = 1;
        print_det(\%configuration, \%detector);
